...
//			}
        ...
        if (mask.isInMask(DBChar.MapTransfer)) {
            for (int i = 0; i < 5; i++) {
                outPacket.encodeInt(0xDEADBEED);
            }
            for (int i = 0; i < 10; i++) {
                outPacket.encodeInt(0xDEADBEED);
            }
...
//			outPacket.encodeInt(getMonsterBookInfo().getSetID()); // removed 196
        ...
        if (mask.isInMask(DBChar.RedLeafInfo)) { // new 188
            outPacket.encodeInt(-1);
        }
...
//        chatMessage("[addToBaseStat] id: %s", Integer.toString(si.getSkillId()));
    ...
    public void warp(Field toField, Portal portal, boolean characterData) {
        ...
        initFriendStatus();
        getClient().getChannelInstance().broadcastPacket(WvsContext.broadcastMessage(4, getClient().getChannel(), "Welcome to v206 MapleStory Development", false));
